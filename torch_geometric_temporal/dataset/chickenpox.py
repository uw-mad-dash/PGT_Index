...
class seq_seq_ChickenpoxDatasetLoader(object):
    ...
    def _get_targets_and_features(self):
        ...
        self.features = [
            ...
            for i in range(stacked_target.shape[0] - self.lags - self.lags + 1)
        ]
        self.features = np.expand_dims(self.features, axis=-1)
        
        self.targets = [
            stacked_target[i + self.lags: i + self.lags + self.lags, :]
            for i in range(stacked_target.shape[0] - self.lags - self.lags + 1)
        ]
