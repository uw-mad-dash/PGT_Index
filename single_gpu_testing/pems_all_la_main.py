...
def train(train_dataloader, val_dataloader, mean, std, epochs, seq_length, num_nodes, num_features, allGPU=False, debug=False):
    ...
    sensor_ids, sensor_id_to_ind, adj_mx = load_graph_data("data/adj_mx_pems-all-la.pkl")
...
def main():
    ...
    if args.mode == "base":
        # preprocess data
        pre_t1 = time.time()
        x_tensor,y_tensor = benchmark_preprocess("data/pems-all-la.h5","pems-all-la")
        ...
        if allGPU == True:
            data, mean, std, x_train, x_val, x_test = IndexDataset.preprocess("data/pems-all-la.h5", 12,"pems-all-la", h5_key="df", add_time_in_day=True,gpu=0)
        else:
            data, mean, std, x_train, x_val, x_test = IndexDataset.preprocess("data/pems-all-la.h5",12,"pems-all-la", h5_key="df",add_time_in_day=True)
